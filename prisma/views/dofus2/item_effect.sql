  ...
  (
    ...
    LEFT JOIN (
      SELECT
        ...
        i."iconId" AS icon_id,
        ie."effectId" AS effect_id,
        CASE
          WHEN (ie."diceNum" > 0) THEN ie."diceNum"
          ELSE ie.value
        END AS value
      FROM
        (
          dofus2_raw.item i
          LEFT JOIN dofus2_raw.item_effectinstancedice ie ON ((ie.d2o_index = i.id))
        )
      WHERE
        (
          (i."typeId" = 78)
    ...
    ) rune ON ((ieff."effectId" = rune.effect_id))
  )
ORDER BY
  ieff.d2o_index,
  ieff.iter_id;