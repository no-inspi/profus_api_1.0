...
router.post('/insert_item_type', async (req: any, res: any) => {
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        let item_type_count = await db.item_type.count(
            {
                where: json_to_push
            }
        )
        ...
        if (item_type_count == 0) {
            const item_type = await db.item_type.create({
                data: json_to_push
            })
            uploaded_record += 1
        }
...
router.post('/insert_item', async (req: any, res: any) => {
    ...
    for (let k = 0; k < result.length; k++) {
        ...
        let item_count = await db.item.count(
            {
                where: obj_item
            }
        )
        ...
        if (item_count == 0) {
            const item_tmp = await db.item.create({
...
router.post('/insert_weapon', async (req: any, res: any) => {
    ...
    for (let k = 0; k < result.length; k++) {
        ...
        obj_item['id'] = k+1
        // console.log(obj_item)
        let weapon_count = await db.weapon.count(
            {
                where: obj_item
            }
        )
        ...
        if (weapon_count == 0) {
            const weapon_tmp = await db.weapon.create({
                data: obj_item as any
            })
            uploaded_record += 1
            console.log('uploaded num : '+k)
